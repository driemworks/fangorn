...
## The Problem: Web3's Gap: Not your keys, not your data

Web3 has given us cryptographically verifiable identity and ownership, but ownership of **what**?

Decentralized Identity != Decentralized Access Control.

When an NFT references a CID you can prove ownserhip of some asset (e.g. asset id #123), but this does not grant access to the actual content! If the content is in IPFS and becomes unpinned, it could be irrecoverable. If it's in a centralized server, then there's little point to owning the NFT. If whichever service makes *meaning* of the NFT shuts down, the owned asset is meaningless.

## The Solution: Intent-Bound Data

Fangorn is a tool for the post-platform economy.

It is a trustless, distributed and permissionless **silent threshold encryption** network that aims to accomplish general purpose, 'practical' *witness encryption*. That is, it acts as a decentralized key management system (DKMS) where data can be encrypted under user provided *intent* instead of a public key. Through an extensible **gadget** framework, our system allows for new mechanisms for *decentralized conditional access control* to be easily implemented and **composed**, allowing the formation of more complex, compound statements.

It uses [a breakthrough in threshold encryption](https://eprint.iacr.org/2024/263) to introduce a new paradigm, which we call `intent-bound data`.


### How it Works

1. A creator encrypts their video under an *intent*, like "You own an asset with id = 123".
2. Upload the ciphertext to storage (e.g. IPFS)
3. Register the intent (on-chain)
4. A consumer buys the NFT and makes a request to fangorn for decryption 
5. The network verifies the condition and outputs *partial decryptions*
6. The consumer decrypts the ciphertext and owns it forever


After registering the intent, the author can go offline . They only need to encrypt once, but it can be decrypted as many times as **their** intent definition allows. 

### What is Witness Encryption?

When an author publishes their book, they can't predict every future reader's identity. Yet, through complicated contracts and distribution deals, 

**Witness encryption** is a type of encryption scheme where data is encrypted for **provable conditions**, with decryption made possible if and only if you can satisfy the condition by providing a **witness**.
...
While DIDs can logically gate access, content is still on centralized servers. The phrase "not your keys, not your crypto" is often said in terms of custodial v.s. non-custodial crypto wallets. The same applies to your data. Not your keys, not your data. 
...
The issue extends beyond the purely digital. Maybe you are a farmer and bought a John Deere tractor, but must rely on the company for repairs . You bought a printer but you must use their proprietary ink cartdidges. The list goes on and on.
...
## The Solution: Eliminate the Gatekeeper
