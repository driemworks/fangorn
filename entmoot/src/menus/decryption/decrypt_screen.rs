        ...
        KeyCode::Enter => {
            ...
            if app.display_password_input {
                ...
                if password.len() == 0 || filename.len() == 0 {
                    ...
                    let witness_string = &prepare_witness_string(app, password)
                        .await
                        .expect("Something went wrong when creating witness string");
                    let contract_addr =
                ...
                if filename.len() == 0 {
                    ...
                    let config_path = String::from("config.txt");
                    let witness_string = &prepare_witness_string(app, String::from(""))
                        .await
                        .expect("Something went wrong when creating witness string");
                    let contract_addr =
                        String::from("5Ccuf8QBBoqZtUPFTxwixMd9mfHLUmXhRvNfBdEU7uL1ApR7");
...
async fn prepare_witness_string(app: &mut App, password: String) -> Result<String, KeystoreError> {
    ...
    app.substrate_backend
        .as_mut()
        .unwrap()
        .update_signer(signer.clone());
    ...
    if app.display_password_input && app.display_contract_address_input && app.sr25519_intent {
        ...
        witness_string = String::from(format!(
            "{},{},{}{}",
            password, pubkey, pubkey, signature_hex
        ));
    } else if !app.display_password_input
...
pub fn render_decrypt_info(app: &mut App, frame: &mut Frame) {
    ...
    let [_, filename_area_vert, password_area_vert, footer_area] =
